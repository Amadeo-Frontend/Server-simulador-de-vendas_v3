...
import { loadProducts, saveProducts } from "../store/products.store.js";

/**
 * Seed inicial caso ainda não exista /data/products.json
 */
const SEED = [
  {
    id: 1,
    ...
    preco_venda_B: 29.09,
    preco_venda_C: 28.21,
    preco_venda_A_prazo: 30.93,
    preco_venda_B_prazo: 29.99,
    preco_venda_C_prazo: 29.08,
    bonificacao_unitaria: 0,
  },
];

// Carrega do disco; se vazio/inexistente, usa seed e grava
let products = [];
try {
  ...
  // Em caso de erro de leitura, inicia com seed e tenta salvar
  products = SEED.slice();
  await saveProducts(products);
}
...
/** ===== EXPORT (RAW JSON – útil p/ inspeção e backup manual) ===== */
router.get("/export/raw", (_req, res) => {
